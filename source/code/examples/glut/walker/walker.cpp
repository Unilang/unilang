...
{
  std::string real_filename = "code/examples/glut/walker/";
...
void WriteCSetToFile(char *filename)
{
  std::string real_filename = "code/examples/glut/walker/";
  real_filename += filename;
  FILE *outfile = fopen(real_filename.c_str(), "w+");
  int joint, point;
...
int MakeLoadAndSaveMenus(void)
{
  DIR *dirp = opendir("code/examples/glut/walker");
...
int main(int argc, char** argv)
{
  ...
  /* read first curve if there was one */
  if (MakeLoadAndSaveMenus() > 0){
    HandleLoadMenu(0);
  }
